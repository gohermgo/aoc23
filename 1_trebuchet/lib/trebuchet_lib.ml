...
let log_folder = "logs/"
type log_variant = [`Input | `Output | `Result ]
...
let get_result : string = let sum = List.fold parse_input ~init:Int64.zero ~f:Int64.(+) in Int64.to_string sum

(* Probably a wiser way to do this *)
let generate_log : (_) = function
    ...
    | `Result -> let content = get_result in Stdio.Out_channel.write_all (get_log_path `Result) ~data:content
